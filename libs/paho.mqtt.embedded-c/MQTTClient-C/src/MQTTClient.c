...
static int sendPacket(MQTTClient *c, int length, Timer *timer) {
        ...
        while (sent < length && !TimerIsExpired(timer)) {
                rc = c->ipstack->mqttwrite(c->ipstack, (char*) &c->buf[sent],
                                length-sent, TimerLeftMS(timer));
...
static int readPacket(MQTTClient *c, Timer *timer) {
        ...
        if (rem_len > 0
                        && (rc = c->ipstack->mqttread(c->ipstack,
                                        c->readbuf + len, rem_len,
                ...
                if (c->ping_outstanding)
                        ...
                        if (len > 0
                                        && (rc = sendPacket(c, len, &timer))
                                                        == SENT_SUCCESS) // send the ping packet
...
int cycle(MQTTClient *c, Timer *timer) {
        ...
        case PUBREL: {
                ...
                if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf,
                        ...
                        rc = FAILURE;
                                ...
                                mypacketid)) <= 0)
                        rc = FAILURE;
                else if ((rc = sendPacket(c, len, timer)) != SENT_SUCCESS) // send the PUBREL packet
        ...
        if (keepalive(c) != SENT_SUCCESS) {
                //check only keepalive FAILURE status so that previous FAILURE status can be considered as FAULT
                rc = FAILURE;
        }

        exit: if (rc == SENT_SUCCESS)
                rc = packet_type;
...
int MQTTConnectWithResults(MQTTClient *c, MQTTPacket_connectData *options,
	  ...
	  MutexLock(&c->mutex);
        ...
        if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0)
                goto exit;
        if ((rc = sendPacket(c, len, &connect_timer)) != SENT_SUCCESS) // send the connect packet
                goto exit;
        // there was a problem
#if 1
...
int MQTTSetMessageHandler(MQTTClient *c, const char *topicFilter,
        ...
        for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i) {
                if (c->messageHandlers[i].topicFilter != NULL
                                && strcmp(c->messageHandlers[i].topicFilter,
                                                topicFilter) == 0) {
                        ...
                        rc = SENT_SUCCESS; /* return i when adding new subscription */
                        break;
                }
        }
...
int MQTTSubscribeWithResults(MQTTClient *c, const char *topicFilter,
	  ...
	  MutexLock(&c->mutex);
        ...
        if (len <= 0)
                goto exit;
        if ((rc = sendPacket(c, len, &timer)) != SENT_SUCCESS) // send the subscribe packet
                goto exit;
        // there was a problem
        ...
        if (waitfor(c, SUBACK, &timer) == SUBACK)      // wait for suback
                ...
                if (MQTTDeserialize_suback(&mypacketid, 1, &count,
                        ...
                        if (data->grantedQoS != 0x80)
                                rc = MQTTSetMessageHandler(c, topicFilter,
                                                messageHandler);
                }
        } else
                rc = FAILURE;
...
int MQTTSubscribe(MQTTClient *c, const char *topicFilter, enum QoS qos,
                messageHandler messageHandler) {
        MQTTSubackData data;
        return MQTTSubscribeWithResults(c, topicFilter, qos, messageHandler,
                        &data);
}
